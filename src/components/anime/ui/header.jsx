...
import { Search, Bookmark, Home, Tv, Film, Cat } from 'lucide-react';
...
export default function AnimeHeader() {
  ...
  return (
    <>
      ...
      {/* Mobile Nav Bar */}
      <nav
        className="fixed bottom-0 left-0 w-full flex justify-around items-center py-3 bg-[#232323ab] backdrop-blur-lg text-white md:hidden z-50"
      >
        {[...navItems, ...headerIcons].map(({ to, label, icon: Icon }, idx) => (
          <NavLink
            key={idx}
            to={to}
            end={to === '/'}
